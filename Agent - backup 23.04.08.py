...
class Agent:
    ...
    def Solve(self,problem):
        ...
        if problem.problemType == '2x2':
            ...
            relationships = self.relation(problem, Pix)
            ...
            result = self.select(problem, relationships, Ratios, TPM, STPM)
        ...
        ## 3X3 MATRICES
            ...
            result = self.select(problem, relations, Ratios, TPM, STPM)
    ...
    # Method that finds the following relations in the frames: identical, symmetric,
    def relation(self, problem, Pix):
        relationships = {'reflection_LR': [], 'reflection_UD': [], 'reflection_diag': []}  # Reflection (Mirror) left-right & up-down
    ...
    def Match_relations(self, problem, relationships, Pix):
        ...
        if problem.problemType == '2x2':
        # TODO: Extend this part to 2x2 matrices
            pass

        #
        elif problem.problemType == '3x3':
            numbers = ['1', '2', '3', '4', '5', '6', '7', '8']
            if sum(relationships['reflection_LR']) == len(relationships['reflection_LR']):
                for N in numbers:
                    # REFLECTION ALONG VERTICAL AXIS
                    # 'G' reflection of '#'
                    # Invert boolean: 1 is black, 0 is white
            ...
            if sum(relationships['reflection_UD']) == len(relationships['reflection_UD']):
                for N in numbers:
                    # REFLECTION ALONG HORIZONTAL AXIS
                    # 'C' reflection of '#'
                    # Invert boolean: 1 is black, 0 is white
            ...
            if sum(relationships['reflection_diag']) == len(relationships['reflection_diag']):
                for N in numbers:
                    # REFLECTION ALONG DIAG AXIS
                    # 'A' reflection of '#'
                    # Invert boolean: 1 is black, 0 is white
    ...
    def STPM(self,Pix,problem):
        # 1 is white, 0 is black, this will be reversed in 'Match_STPM'
        if problem.problemType == '2x2':
            Pix_STPM = {'AB': 0, 'AC': 0}
    ...
    # Method that finds the best indices based on the results given by the STPM
    def Match_STPM(self, STPM):
        ind_STPM = []
        sqdif_STPM = []
        TP_STPM = {}
        Ratios = {'DPR': {}, 'IPR': {}}
    ...
    def select(self,problem, relations, Ratios, TPM, STPM):
        ...
        if problem.problemType == '2x2':
            ...
            # As a 1st pass, see correlation coefficients of TPMs
            ind_corr, corr_coef = self.CorrCoef_TPM(TPM)
            if len(ind_corr) == 1:
                # Almost perfect correlation found, return index and add 1, as indices start with 0.
                result = ind_corr[0] + 1
                return result

            # As a 2nd pass, use STPM to find indices that lead to good match when combining frames
            ind_STPM, sqdif_STPM  = self.Match_STPM(STPM)
